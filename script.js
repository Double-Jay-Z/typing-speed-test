...
function showText(texts) {
  let randomIndex = Math.floor(Math.random() * texts.length); // Generate random array index
  let randomSplice = texts.splice(randomIndex, 1); // Throwing out randomly selected sentence, that the end could be reached
...
function lap() {
  laps.innerHTML += "<li>" + leadingZero(timer[0]) + ":" + leadingZero(timer[1]) + ":" + leadingZero(timer[2]) + "</li>";
}

function runOtherTimer(){
  let otherCurrentTime = leadingZero(otherTimer[0]) + ":" + leadingZero(otherTimer[1]) + ":" + leadingZero(otherTimer[2]);
...
function numberOfEntries() {
  let noe = testArea.value.length; // noe = Number Of Entries
  wpmDiv.innerHTML = "<p>" + "Number of characters including spaces: " + noe + "</p>";
}
...
function totalCharacters() {
    ...
    let getSeconds = (minutesParsed * 60) + secondsParsed;
  	let getMinuteFromSeconds = Math.round((getSeconds / 60) * 100) / 100; // rounding minutes to 2 decimals
    let wpm = Math.floor(now / getMinuteFromSeconds);
    wpmSpan.innerHTML = "Converted minutes: " + getMinuteFromSeconds + " Number of characters: " + charactersCount + ", number of words: " + now + "<br>" + "Your WPM: " + "<b>" + wpm + "</b>";
}

// Match the text entered with the provided text on the page
function spellCheck() {
    let textEntered = testArea.value;
    let originTextMatch = originText.innerHTML.substring(0,textEntered.length); // getting the parts of textEntered
